...
import './SolarisTourModal.css'

// Import all 14 thumbnails for the selection tray
...
// Import all 14 high-resolution 360-degree panoramas
import pano1 from '../assets/pano1.png'
import pano2 from '../assets/pano2.png'
import pano3 from '../assets/pano3.png'
import pano4 from '../assets/pano4.png'
import pano5 from '../assets/pano5.png'
import pano6 from '../assets/pano6.png'
import pano7 from '../assets/pano7.png'
import pano8 from '../assets/pano8.png'
import pano9 from '../assets/pano9.png'
import pano10 from '../assets/pano10.png'
import pano11 from '../assets/pano11.png'
import pano12 from '../assets/pano12.png'
import pano13 from '../assets/pano13.png'
import pano14 from '../assets/pano14.png'

const scenes = [
  { img: thumb1, pano: pano1, name: 'Driveway Entrance' },
  { img: thumb2, pano: pano2, name: 'Underground Corridor' },
  { img: thumb3, pano: pano3, name: 'Main Hallway' },
  { img: thumb4, pano: pano4, name: 'Living Room' },
  { img: thumb5, pano: pano5, name: 'Fitness Gym' },
  { img: thumb6, pano: pano6, name: 'Guest Bedroom' },
  { img: thumb7, pano: pano7, name: 'Master Bathroom' },
  { img: thumb8, pano: pano8, name: 'Kitchen & Dining' },
  { img: thumb9, pano: pano9, name: 'Sun Lounge' },
  { img: thumb10, pano: pano10, name: 'Swimming Pool Area' },
  { img: thumb11, pano: pano11, name: 'Outdoor Terrace' },
  { img: thumb12, pano: pano12, name: 'Walkway Courtyard' },
  { img: thumb13, pano: pano13, name: 'Garage Bay' },
  { img: thumb14, pano: pano14, name: 'Rooftop Lounge' }
]

export default function SolarisTourModal({ isOpen, onClose }) {
    ...
    const render = (time) => {
      ...
      if (!camera.isDragging) {
        ...
        if (Math.abs(camera.velYaw) < 0.0002) {
          camera.velYaw = 0.0002 // slow spin
        }
      } else {
        camera.velYaw *= 0.8
        camera.velPitch *= 0.8
  ...
  return (
    <div className="solaris-modal-backdrop" ref={containerRef}>
      {/* Top Header navbar */}
      <div className="solaris-header">
        <button className="solaris-close-btn" onClick={onClose} aria-label="Close Tour">
      ...
      {/* Main showcase area */}
      <div
        className={`solaris-viewer ${transitioning ? 'solaris-viewer--fade' : ''}`}
      ...
      >
        <canvas ref={canvasRef} className="solaris-canvas" />

        {/* Loading Spinner */}
        {!isReady && (
          <div className="solaris-loader">
            <div className="solaris-spinner" />
        ...
        {hotspotList.map((hs, i) => (
          <button
            key={i}
            className={`solaris-street-arrow solaris-street-arrow--${hs.type}`}
            style={{
              left: `${hs.x}px`,
              top: `${hs.y}px`
            }}
        ...
        {vrMode && <div className="solaris-vr-divider" />}
      </div>

      {/* Dynamic Navigation & Thumbnail panel */}
      <div className="solaris-controls-container">
        {/* Horizontal Thumbnails Tray */}
        {showThumbs && (
          <div className="solaris-thumbnails-tray">
            <div className="solaris-thumbnails-scroll">
              {scenes.map((scene, idx) => (
                <div
                  key={idx}
                  className={`solaris-thumb-card ${idx === currentIdx ? 'active' : ''}`}
                  onClick={() => changeScene(idx)}
                >
                  <img src={scene.img} alt={scene.name} className="solaris-thumb-img" />
                  <span className="solaris-thumb-label">{idx + 1}</span>
                </div>
              ))}
            </div>
          </div>
        ...
        {/* Bottom Control Bar */}
        <div className="solaris-control-bar">
